### Day 7 -- Simple Decision Tree using GINI
"""
This code is a simple improvement of DecisionTree

Yesterday I created a DecisionTreeClassifier from scratch using the entropy as a measure of information gain based of the information theory of Shannon

Today, we are going to use Gini which gives us a measur of disorder
Gini is equal to : 1 -SUM(p(i)**2) at a node
It measures the probability that a data point will be randomly wrongfully labeled

In this code, we will simply take our previous code and use Gini instead of Entropy, no big changes but this is an important step because most of DecisionTree use Gini because it is faster and computational efficiency is key in ML
...
"""
df = pd.read_csv("/Users/guilhemcreus/Desktop/Github/WEEK 1/CarDealerData.csv")
...
2) Gini index and Gini:
    -Gini of a node is equal to 1 - SUM(p(i)**2) with p(i) the probability of a data point to be classified rightfully at this node (i.e, this is number of rightfully classified data points at this node/all data points at this node)
    -Gini index = gini(parent) - SUM(wi * gini(child(i))) with wi the weight of data points sent to the child(i) (amount of data points sent to child(i)/total datapoints at parent node); and child(i) the left or right child node

So the Gini index of a whole tree is the Gini index of the root which will itself call the Gini index of the child nodes etc

With this in mind, we understand now what error metric we will seek to minimize
"""

class Node():
    def __init__(self, feature_index=None, threshold=None, left=None, right=None, gini_index=None, value=None):
        '''
        ...
        Ags :
            ...
            gini_index = the gini index of the node from the split
...
class DecisionTreeClassifier():
    ...
    def gini(self, Y_rows : list[float]) -> float:
        """
        Ags :
            Y_rows = the Y values in a list format as input

        Return the gini at that node
        """
    ...
    def gini_index(self, parent : list[float], left_child : list[float], right_child : list[float]) -> float:
        """
        Ags :
            parent = the actual Y values at the current node at which we are looking to calculate the gini index
            left_child = the left child Y values
            right_child = the right child Y values


        Return the gini index at that node
        """
        weight_left = len(left_child) / len(parent)
        weight_right = len(right_child) / len(parent)
        gini_index = self.gini(parent) - (weight_left*self.gini(left_child) + weight_right*self.gini(right_child))
        return gini_index
    ...
    def split(self, X : list[list[float]], Y_rows : list[float], feature_index : int, threshold : float) -> tuple[list[list[float]], list[float], list[list[float]], list[float]]:
        """
        Args:
    ...
    def get_best_split(self, X : list[list[float]], Y_rows : list[float]) -> dict:
        ...
        Return a dict containing the best gini_index by looking at all gini_index possible (brute force); the dict has the following attributes :
        - feature_index
        - threshold
        ...
        best_split = {}
        max_gini_index = -float("inf") # We start with the minimum float possible to prevent inputting by mistake a gini index that would be higher than every gini index the tree could provide
        ...
        for feature_index in range(number_features):
            ...
            for threshold in unique_values:
                ...
                current_gini_index = self.gini_index(Y_rows, Y_left, Y_right)
                if current_gini_index > max_gini_index:
                    best_split = {
                    ...
                    max_gini_index = current_gini_index
    ...
    def build_tree(self, X : list[list[float]], Y_rows : list[float], current_depth : int = 0) -> Node:
        ...
        Return a Tree with the best gini index
        """
        ...
        if number_samples >= self.minimum_samples_split and current_depth < self.maximum_depth:
            ...
            We also check that the best possible gini index is positive; if you remind yourself the formula of the gini index, you know that if the best gini index is negative or zero, then no need to try to split further because it would tend to degrade the ability of the tree to correctly predict values
            """
            if best and best.get("gini_index", 0) > 0:
...
"""
Using information gain, our model was more precise and we sas that in the preivous two predictions; this is an advantage of information gain, but globally, gini index does the work pretty good for an huge decrease in computational requirements
This is why gini index is favoured because the small loss due to less precise forecast is outbalances by less resources needed to fit it
"""
